resource "aws_s3_bucket" "media_backup" {
  ...
  tags   = { Name = "${var.environment}-media-backup" }
}
...
resource "aws_s3_bucket" "code_backup" {
  ...
  tags   = { Name = "${var.environment}-code-backup" }
}
...
resource "aws_s3_bucket" "dr_backup" {
  ...
  tags   = { Name = "${var.environment}-dr-backup" }
}
...
resource "aws_cloudfront_distribution" "dr" {
  ...
  tags = { Name = "${var.environment}-dr-cloudfront" }
}
...
resource "aws_s3_bucket_policy" "dr_backup" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = { StringEquals = { "AWS:SourceArn" = aws_cloudfront_distribution.dr.arn } }
    }]
  })
}
